...
if (isset($_POST['favorite']))
{
    ...
    $favorite_sql="INSERT INTO favorite (episode_id,user_id) VALUES ({$the_episode_id}, {$user_id})";
    $add_favorite=mysqli_query($connection,$favorite_sql);
    confirmQuery($add_favorite);
}
if (isset($_POST['unfavorite']))
{
    $query3="DELETE FROM favorite WHERE episode_id={$the_episode_id} AND user_id={$_SESSION['user_id']}";
...
{
    $fav_table="SELECT * FROM favorite WHERE episode_id={$the_episode_id} AND user_id={$_SESSION['user_id']}";
...
{
    $user_id=$_SESSION['user_id'];
    $watch_sql="INSERT INTO watchlist (episode_id,user_id) VALUES ({$the_episode_id}, {$user_id})";
...
if (isset($_SESSION['user_id']))
{
    $watch_table="SELECT * FROM watchlist WHERE episode_id={$the_episode_id} AND user_id={$_SESSION['user_id']}";
